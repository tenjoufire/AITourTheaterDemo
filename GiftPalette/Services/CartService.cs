...
public class CartService : ICartService
{
    ...
    public async Task<Cart> GetCartAsync()
    {
        try
        {
            var response = await _httpClient.GetAsync($"http://localhost:5261/api/cart/{CartId}");
    ...
    public async Task<Cart> AddToCartAsync(int productId, int quantity)
    {
        try
        {
            ...
            var response = await _httpClient.PostAsync($"http://localhost:5261/api/cart/{CartId}/add", content);
    ...
    public async Task<Cart> UpdateCartItemAsync(int productId, int quantity)
    {
        try
        {
            ...
            var response = await _httpClient.PutAsync($"http://localhost:5261/api/cart/{CartId}/update", content);
    ...
    public async Task<Cart> RemoveFromCartAsync(int productId)
    {
        try
        {
            var response = await _httpClient.DeleteAsync($"http://localhost:5261/api/cart/{CartId}/remove/{productId}");
    ...
    public async Task<bool> ClearCartAsync()
    {
        try
        {
            var response = await _httpClient.DeleteAsync($"http://localhost:5261/api/cart/{CartId}/clear");
